...
export default function SolicitationDetail() {
  ...
  const handleAction = async (action, observation) => {
  ...
  const canShowApprove = hasRole(['MANAGER']) && solicitation.status === 'SUBMITTED';
  const canShowReject = hasRole(['MANAGER']) && solicitation.status === 'SUBMITTED';
  const canShowPay = hasRole(['FINANCE']) && solicitation.status === 'APPROVED';
  const canShowCancel = (isOwner && solicitation.status === 'DRAFT') || 
                         (hasRole(['MANAGER']) && solicitation.status === 'SUBMITTED');
